...
After a successful response, the input method checks GitHub for the latest stable release at most once per day. A failed automatic check waits one hour before retrying, so a temporary offline period does not suppress update discovery for a full day. When a newer version is available, the input menu marks the settings entry and the settings panel downloads the matching ZIP installer directly from this repository's fixed GitHub Releases origin. It prefers a signed ZIP when available, otherwise selects the clearly marked unsigned ZIP; if no exact matching archive is present, it safely falls back to the fixed release page. You can also check manually from the settings panel.
