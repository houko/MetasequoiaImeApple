...
The current release supports full-pinyin composition, live candidates from the official Metasequoia dictionary, auxiliary-code hints beside pinyin candidates, candidate selection through the native candidate panel or number keys 1–9, Space to commit the leading candidate, Return to commit raw input, Backspace, Escape, composition commit on focus changes, Shift+Space switching between Chinese and direct English input, and an optional full-width mode toggled with Option+Shift+H. A native draggable 悬浮状态栏 keeps Chinese/English, punctuation, full-width, and simplified/traditional output state visible and provides one-click access to settings; it can be hidden from the Appearance page. When full-width mode is enabled, idle ASCII letters, digits, punctuation, and spaces are converted to their Unicode full-width forms without changing pinyin composition. Simplified output remains the default; the input menu and floating toolbar can switch visible candidates and committed Chinese text to traditional characters without changing dictionary keys or learning data.
...
In System Settings > Keyboard > Text Input > Edit, select 水杉输入法 and choose its settings action to open the native 水杉输入法设置 panel. The panel lets you choose 全拼、小鹤双拼 or 86 五笔, configure unique four-code Wubi candidates to commit automatically, preview and switch the native candidate window between a horizontal row and vertical list, show 5, 7, or 9 candidates per page, choose a small, standard, or large candidate font, select `- / =`, `[ / ]`, or Page Up / Page Down for candidate paging, enable full-pinyin autocorrection and auxiliary codes, independently choose 蓝天小雨点、自然码、首右2.0、首右plus 或小鹤 for full-pinyin and double-pinyin auxiliary codes, switch Chinese punctuation conversion, enable Option+Shift+H full-width input, and control whether selected candidates update learned word frequencies. When auxiliary codes are enabled, pinyin candidates display their hints in parentheses, matching the candidate presentation shown in the product screenshots. These choices are saved for the current user and apply before the next key event when no composition is active; an active composition keeps its original settings until it is committed or cancelled. Additional input and candidate options will be added incrementally.
...
For normal installation, use the ZIP and run its `Install.command`, or use the PKG with the native macOS Installer. Both paths install the current-user bundle and attempt to register and enable 水杉 automatically without logging out or restarting the Mac. If no GUI user is logged in, or macOS blocks an unsigned app, enable it later in System Settings > Keyboard > Text Input > Edit.
